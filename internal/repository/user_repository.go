...
func (r *userRepo) AddUser(ctx context.Context, user *model.User) error {
	...
	err := r.pool.QueryRow(ctx, queryAddUser, user.FullName, user.Email, user.PasswordHash, user.Role, user.IsActive, user.CreatedAt, user.UpdatedAt).
		Scan(&user.ID)
	if err != nil {
...
func (r *userRepo) GetRefreshToken(ctx context.Context, token string) (model.RefreshToken, error) {
	...
	err := r.pool.QueryRow(ctx, queryGetRefreshToken, token).Scan(&rt.ID, &rt.UserID, &rt.Token, &rt.ExpiresAt, &rt.CreatedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
